...
#define RESPONSE_TYPE "id_token code"
...
START_TEST(test_oidc_hybrid_id_token_code_redirect_login_post)
{
...
START_TEST(test_oidc_hybrid_id_token_code_valid_post)
{
  struct _u_response resp;
  char * id_token, * code, ** id_token_split = NULL, * str_payload, at_hash[33], at_hash_encoded[64];
...
START_TEST(test_oidc_hybrid_id_token_code_scope_grant_none)
{
  ...
  // Try to get code
  code_req.http_verb = strdup("GET");
  code_req.http_url = msprintf("%s/oidc/auth?response_type=%s&state=xyzabcd&nonce=nonce1234&g_continue&client_id=%s&redirect_uri=..%%2f..%%2ftest-oidc.html%%3fparam%%3dclient1_cb1&scope=%s", SERVER_URI, RESPONSE_TYPE, CLIENT, SCOPE_LIST);
  ck_assert_int_eq(ulfius_send_http_request(&code_req, &code_resp), U_OK);
  ck_assert_int_eq(code_resp.status, 302);
  ck_assert_ptr_ne(o_strstr(u_map_get(code_resp.map_header, "Location"), "id_token="), NULL);
...
static Suite *glewlwyd_suite(void)
{
  ...
  s = suite_create("Glewlwyd oidc hybrid id_token code");
  tc_core = tcase_create("test_oidc_hybrid_id_token_code");
  tcase_add_test(tc_core, test_oidc_hybrid_id_token_code_redirect_login);
  tcase_add_test(tc_core, test_oidc_hybrid_id_token_code_redirect_login_post);
  tcase_add_test(tc_core, test_oidc_hybrid_id_token_code_valid);
  tcase_add_test(tc_core, test_oidc_hybrid_id_token_code_valid_post);
