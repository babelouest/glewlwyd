...
START_TEST(test_oidc_delete_token_client_confidential_token_invalid)
{
...
START_TEST(test_oidc_delete_token_client_confidential_client_invalid)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
  ...
  int res = run_simple_test(NULL, "POST", url, CLIENT, "error", NULL, &body, 400, NULL, NULL, NULL);
  o_free(url);
  u_map_clean(&body);
  ck_assert_int_eq(res, 1);
}
...
START_TEST(test_oidc_delete_token_client_confidential_no_client)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_delete_token_client_confidential_ok)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_delete_token_client_confidential_token_already_deleted)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tcase_add_test(tc_core, test_oidc_delete_token_client_confidential_ok);
  tcase_add_test(tc_core, test_oidc_delete_token_client_confidential_token_already_deleted);
