...
START_TEST(test_oidc_delete_token_token_invalid)
{
...
START_TEST(test_oidc_delete_token_client_invalid)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_delete_token_no_client)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_delete_token_ok)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_delete_token_token_already_deleted)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tcase_add_test(tc_core, test_oidc_delete_token_client_invalid);
  tcase_add_test(tc_core, test_oidc_delete_token_no_client);
