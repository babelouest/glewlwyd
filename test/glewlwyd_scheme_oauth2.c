...
START_TEST(test_glwd_scheme_oauth2_irl_auth_invalid_userid)
{
  struct _u_instance instance;
  ck_assert_int_eq(ulfius_init_instance(&instance, PROVIDER_PORT, NULL, NULL), U_OK);
  ck_assert_int_eq(ulfius_add_endpoint_by_val(&instance, "POST", NULL, "/token", 0, &callback_token_ok, NULL), U_OK);
...
START_TEST(test_glwd_scheme_oauth2_irl_auth_error_state)
{
  struct _u_request req;
  struct _u_response resp;
  json_t * j_parameters, * j_response;
...
START_TEST(test_glwd_scheme_oauth2_irl_auth_ok)
{
  struct _u_instance instance;
  ck_assert_int_eq(ulfius_init_instance(&instance, PROVIDER_PORT, NULL, NULL), U_OK);
  ck_assert_int_eq(ulfius_add_endpoint_by_val(&instance, "POST", NULL, "/token", 0, &callback_token_ok, NULL), U_OK);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tcase_add_test(tc_core, test_glwd_scheme_oauth2_irl_auth_invalid_userid);
  tcase_add_test(tc_core, test_glwd_scheme_oauth2_irl_auth_error_state);
  tcase_add_test(tc_core, test_glwd_scheme_oauth2_irl_auth_error_redirect_to);
  tcase_add_test(tc_core, test_glwd_scheme_oauth2_irl_auth_ok);
