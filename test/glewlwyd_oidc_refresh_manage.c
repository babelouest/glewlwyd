...
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI), * token_hash, * token_hash_encoded;
  struct _u_map body;
  struct _u_request user_req;
  struct _u_response resp;
  int res;
...
START_TEST(test_oidc_refresh_manage_delete_all_ok)
{
  ...
  user_req.http_url = msprintf(SERVER_URI "/oidc/token/");
  ck_assert_int_eq(ulfius_send_http_request(&user_req, &resp), U_OK);
  ck_assert_int_eq(resp.status, 200);
  j_body = ulfius_get_json_body_response(&resp, NULL);
  ck_assert_ptr_ne(j_body, NULL);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tcase_add_test(tc_core, test_oidc_refresh_manage_delete_all_ok);
...
int main(int argc, char *argv[])
{
  ...
  if (res == U_OK && auth_resp.status == 200) {
    ...
    y_log_message(Y_LOG_LEVEL_INFO, "User %s authenticated - refresh_token", USERNAME);
  ...
  if (res == U_OK && auth_resp.status == 200) {
    ...
    y_log_message(Y_LOG_LEVEL_INFO, "User %s authenticated - refresh_token_all", USERNAME);
    json_decref(json_body);
    do_test = 1;
  }
  ulfius_clean_response(&auth_resp);
