...
START_TEST(test_glwd_admin_mod_plugin_with_errors)
{
  ...
  url = msprintf(SERVER_URI "/mod/plugin/%s/enable", name);
  ck_assert_int_eq(run_simple_test(&admin_req, "PUT", url, NULL, NULL, j_parameters, NULL, 400, NULL, NULL, NULL), 1);
  o_free(url);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tcase_add_test(tc_core, test_glwd_admin_mod_plugin_with_errors);
