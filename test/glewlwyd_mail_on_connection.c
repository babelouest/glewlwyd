...
START_TEST(test_glwd_mail_on_connection_add_users)
{
  ...
  ck_assert_ptr_ne(NULL, (j_body = json_pack("{sssssss{so}}", "username", USER1, "scheme_type", "mock", "scheme_name", "mock_scheme_42", "value", "register", json_true())));
  ...
  ck_assert_ptr_ne(NULL, (j_body = json_pack("{sssssss{so}}", "username", USER2, "scheme_type", "mock", "scheme_name", "mock_scheme_88", "value", "register", json_true())));
  ...
  ck_assert_ptr_ne(NULL, (j_body = json_pack("{sssssss{so}}", "username", USER3, "scheme_type", "mock", "scheme_name", "mock_scheme_95", "value", "register", json_true())));
...
START_TEST(test_glwd_mail_on_connection_remove_users)
{
  ...
  j_body = json_pack("{sssssss{so}}", "username", USER2, "scheme_type", "mock", "scheme_name", "mock_scheme_88", "value", "register", json_false());
  ck_assert_int_eq(run_simple_test(&admin_req, "POST", SERVER_URI "/delegate/" USER2 "/profile/scheme/register/", NULL, NULL, j_body, NULL, 200, NULL, NULL, NULL), 1);
  json_decref(j_body);

  j_body = json_pack("{sssssss{so}}", "username", USER3, "scheme_type", "mock", "scheme_name", "mock_scheme_95", "value", "register", json_false());
  ck_assert_int_eq(run_simple_test(&admin_req, "POST", SERVER_URI "/delegate/" USER3 "/profile/scheme/register/", NULL, NULL, j_body, NULL, 200, NULL, NULL, NULL), 1);
  json_decref(j_body);
...
START_TEST(test_glwd_mail_on_connection_email_ip_ok)
{
                              ...
                              "value",
                                ...
                                "templates",
                                  "en",
                                    "defaultLang", json_true(),
                                    "subject", SUBJECT,
                                    "body-pattern", BODY_PATTERN_IP "{IP}");
  ck_assert_int_eq(run_simple_test(&admin_req, "PUT", SERVER_URI "/misc/" CONFIG_NAME, NULL, NULL, j_body, NULL, 200, NULL, NULL, NULL), 1);
  json_decref(j_body);
  ...
  ck_assert_int_gt(o_strlen(manager.mail_data), 0);
...
START_TEST(test_glwd_mail_on_connection_reconnection_no_email_ok)
{
  ...
  cookie = o_strdup(resp.map_cookie[0].value);
  ...
  j_body = json_pack("{sss{so ss si ss ss ss s{s{sossss}}}}",
                     "type", CONFIG_TYPE,
                     "value",
                       "enabled", json_true(),
                       "host", HOST,
  ...
  ck_assert_int_eq(ulfius_set_request_properties(&req, U_OPT_HTTP_VERB, "POST",
                                                       U_OPT_HTTP_URL, SERVER_URI "/auth/",
                                                       U_OPT_JSON_BODY, j_body,
                                                       U_OPT_COOKIE_PARAMETER, "GLEWLWYD2_SESSION_ID", cookie,
                                                       U_OPT_NONE), U_OK);
  json_decref(j_body);
  ck_assert_int_eq(ulfius_send_http_request(&req, NULL), U_OK);
  ulfius_clean_request(&req);

  ulfius_init_request(&req);
  j_body = json_pack("{ssss}", "username", USER2, "password", USER_PASSWORD);
  ck_assert_int_eq(ulfius_set_request_properties(&req, U_OPT_HTTP_VERB, "POST",
                                                       U_OPT_HTTP_URL, SERVER_URI "/auth/",
                                                       U_OPT_JSON_BODY, j_body,
                                                       U_OPT_COOKIE_PARAMETER, "GLEWLWYD2_SESSION_ID", cookie,
...
START_TEST(test_glwd_mail_on_connection_reconnection_mfa_no_email_ok)
{
  ...
  ulfius_init_request(&req);
  j_body = json_pack("{sssssss{ss}}", "username", USER2, "scheme_type", "mock", "scheme_name", "mock_scheme_88", "value", "code", "88");
  ck_assert_int_eq(ulfius_set_request_properties(&req, U_OPT_HTTP_VERB, "POST",
                                                       U_OPT_HTTP_URL, SERVER_URI "/auth/",
                                                       U_OPT_JSON_BODY, j_body,
                                                       U_OPT_COOKIE_PARAMETER, "GLEWLWYD2_SESSION_ID", cookie,
