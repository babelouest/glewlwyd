...
#define REDIRECT_URI "../../test-oidc.html?param=client3"
#define REDIRECT_URI_ENCODED "..%2f..%2ftest-oidc.html%3fparam%3dclient3"

struct _u_request user_req;
char * code;

START_TEST(test_oidc_code_client_confidential_code_invalid)
{
...
START_TEST(test_oidc_code_client_confidential_client_invalid)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_code_client_confidential_redirect_uri_invalid)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_code_client_confidential_ok)
{
  char * url = msprintf("%s/oidc/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tcase_add_test(tc_core, test_oidc_code_client_confidential_redirect_uri_invalid);
  tcase_add_test(tc_core, test_oidc_code_client_confidential_ok);
    ...
    if (res == U_OK && auth_resp.status == 200) {
      ...
      if (res == U_OK && auth_resp.status == 200) {
        ...
        if (ulfius_send_http_request(&scope_req, &scope_resp) != U_OK) {
          ...
          user_req.http_url = msprintf("%s/oidc/auth?response_type=code&g_continue&client_id=%s&redirect_uri=%s&state=xyzabcd&scope=%s", SERVER_URI, CLIENT, REDIRECT_URI_ENCODED, SCOPE_LIST);
