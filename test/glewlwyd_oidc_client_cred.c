...
START_TEST(test_oidc_cred_valid)
{
...
START_TEST(test_oidc_cred_valid_reduced_scope)
{
  char * url = msprintf("%s/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_cred_pwd_invalid)
{
  char * url = msprintf("%s/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_cred_client_unauthorized)
{
  char * url = msprintf("%s/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_cred_client_invalid)
{
  char * url = msprintf("%s/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_cred_scope_invalid)
{
  char * url = msprintf("%s/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
START_TEST(test_oidc_cred_empty)
{
  char * url = msprintf("%s/token/", SERVER_URI);
  struct _u_map body;
  u_map_init(&body);
...
static Suite *glewlwyd_suite(void)
{
  ...
  tc_core = tcase_create("test_oidc_cred");
  tcase_add_test(tc_core, test_oidc_cred_valid);
  tcase_add_test(tc_core, test_oidc_cred_valid_reduced_scope);
  tcase_add_test(tc_core, test_oidc_cred_pwd_invalid);
  tcase_add_test(tc_core, test_oidc_cred_client_unauthorized);
  tcase_add_test(tc_core, test_oidc_cred_client_invalid);
  tcase_add_test(tc_core, test_oidc_cred_scope_invalid);
  tcase_add_test(tc_core, test_oidc_cred_empty);
