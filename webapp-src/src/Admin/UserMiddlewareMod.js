...
class UserMiddlewareMod extends Component {
  constructor(props) {
    super(props);
  ...
  addMod(e) {
    messageDispatcher.sendMessage('App', {type: "add", role: "userMiddlewareMod"});
  }
  ...
  editMod(e, mod, index) {
    messageDispatcher.sendMessage('App', {type: "edit", role: "userMiddlewareMod", mod: mod, index: index});
  }

  deleteMod(e, mod) {
    messageDispatcher.sendMessage('App', {type: "delete", role: "userMiddlewareMod", mod: mod});
  }

  moveModUp(e, mod, previousMod) {
    mod.order_rank--;
    previousMod.order_rank++;
    messageDispatcher.sendMessage('App', {type: "swap", role: "userMiddlewareMod", mod: mod, previousMod: previousMod});
  }

  switchModStatus(mod) {
    var action = (mod.enabled?"disable":"enable");
    apiManager.glewlwydRequest("/mod/user_middleware/" + encodeURIComponent(mod.name) + "/" + action + "/", "PUT")
    .then(() => {
    ...
    .always(() => {
      messageDispatcher.sendMessage('App', {type: "refresh", role: "userMiddlewareMod", mod: mod});
    });
  }
    ...
    <table className="table table-responsive table-striped">
      <thead>
        <tr>
          <th colSpan="5">
            <h4>{i18next.t("admin.user-middleware-mod-list-title")}</h4>
          </th>
          <th colSpan="1">
...
export default UserMiddlewareMod;
