...
START_TEST(test_glwd_code_code_invalid)
{
  ...
  int res = run_simple_test(&user_req, "POST", url, CLIENT, CLIENT_PASSWORD, NULL, &body, 400, NULL, NULL, NULL);
  free(url);
  u_map_clean(&body);
...
START_TEST(test_glwd_code_client_invalid)
{
  ...
  free(user_req.http_verb);
  user_req.http_verb = NULL;
  int res = run_simple_test(&user_req, "POST", url, CLIENT, CLIENT_PASSWORD, NULL, &body, 400, NULL, "unauthorized_client", NULL);
  free(url);
  u_map_clean(&body);
	ck_assert_int_eq(res, 1);
}
