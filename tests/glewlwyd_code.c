...
int main(int argc, char *argv[])
{
  ...
  if (res == U_OK) {
    ...
    if (ulfius_send_http_request(&auth_req, NULL) != U_OK) {
      ...
      user_req.http_url = msprintf("%s/auth?response_type=code&client_id=client1_id&redirect_uri=../static/index.html?param=client1_cb1&scope=scope1 scope2", SERVER_URI);
