...
START_TEST(test_glwd_user_refresh_token_revoke_not_found_user)
{
  ...
  u_map_init(&body);
  u_map_put(&body, "token_hash", "not_found");
  int res = run_simple_test(&user_req, "DELETE", url, NULL, NULL, NULL, &body, 404, NULL, NULL, NULL);
  free(url);
  u_map_clean(&body);
	ck_assert_int_eq(res, 1);
}
...
START_TEST(test_glwd_user_refresh_token_revoke_ok_user)
{
  ...
  if (res == U_OK) {
    ...
    u_map_put(admin_req.map_post_body, "session_hash", json_string_value(json_object_get(json_array_get(list_resp.json_body, 0), "session_hash")));
  }
