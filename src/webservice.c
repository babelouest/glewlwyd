...
int callback_glewlwyd_options (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  ulfius_add_header_to_response(response, "Access-Control-Allow-Origin", config->allow_origin);
  ulfius_add_header_to_response(response, "Access-Control-Allow-Methods", config->allow_methods);
  ulfius_add_header_to_response(response, "Access-Control-Allow-Headers", config->allow_headers);
  ulfius_add_header_to_response(response, "Access-Control-Expose-Headers", config->expose_headers);
...
int callback_glewlwyd_delete_user (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (check_result_value(j_search_user, G_OK)) {
    if (delete_user(config, u_map_get(request->map_url, "username"), json_string_value(json_object_get(json_object_get(j_search_user, "user"), "source"))) != G_OK) {
      ...
      y_log_message(Y_LOG_LEVEL_INFO, "Event - User '%s' removed - admin action", u_map_get(request->map_url, "username"));
    }
  } else if (check_result_value(j_search_user, G_ERROR_NOT_FOUND)) {
    response->status = 404;
  } else {
