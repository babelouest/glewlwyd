...
static json_t * check_client_valid(struct _oauth2_config * config, const char * client_id, const char * client_header_login, const char * client_header_password, const char * redirect_uri, unsigned short authorization_type) {
...
static int check_auth_type_access_token_request (const struct _u_request * request, struct _u_response * response, void * user_data) {
  ...
  if (code == NULL || client_id == NULL || redirect_uri == NULL) {
    ...
    j_client = check_client_valid(config, client_id, request->auth_basic_user, request->auth_basic_password, redirect_uri, GLEWLWYD_AUTHORIZATION_TYPE_AUTHORIZATION_CODE);
    if (check_result_value(j_client, G_OK)) {
  ...
  if (refresh_token != NULL && o_strlen(refresh_token)) {
    ...
    if (check_result_value(j_refresh, G_OK)) {
      if (json_object_get(json_object_get(j_refresh, "token"), "client_id") != json_null()) {
        j_client = check_client_valid(config, json_string_value(json_object_get(json_object_get(j_refresh, "token"), "client_id")), request->auth_basic_user, request->auth_basic_password, NULL, GLEWLWYD_AUTHORIZATION_TYPE_REFRESH_TOKEN);
        if (!check_result_value(j_client, G_OK)) {
          has_issues = 1;
    ...
    if (pthread_mutex_init(&((struct _oauth2_config *)*cls)->insert_lock, &mutexattr) != 0) {
      ...
      *cls = NULL;
      ...
      if (((struct _oauth2_config *)*cls)->glewlwyd_resource_config != NULL) {
        ...
        ((struct _oauth2_config *)*cls)->glewlwyd_resource_config->oauth_scope = NULL;
        ((struct _oauth2_config *)*cls)->glewlwyd_resource_config->realm = NULL;
        ((struct _oauth2_config *)*cls)->glewlwyd_resource_config->accept_access_token = 1;
        ((struct _oauth2_config *)*cls)->glewlwyd_resource_config->accept_client_token = 0;
        j_result = check_parameters(((struct _oauth2_config *)*cls)->j_params);
              ...
              if (jwt_autocheck(((struct _oauth2_config *)*cls)) != G_OK) {
                ...
                *cls = NULL;
                ...
                if (config->glewlwyd_callback_add_plugin_endpoint(config, "GET", name, "auth/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_oauth2_authorization, (void*)*cls) != G_OK || 
                   config->glewlwyd_callback_add_plugin_endpoint(config, "POST", name, "token/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_oauth2_token, (void*)*cls) || 
                   ...
                   config->glewlwyd_callback_add_plugin_endpoint(config, "GET", name, "profile/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_oauth2_get_profile, (void*)*cls) || 
                   config->glewlwyd_callback_add_plugin_endpoint(config, "GET", name, "profile/token/", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_oauth2_refresh_token_list_get, (void*)*cls) || 
                   config->glewlwyd_callback_add_plugin_endpoint(config, "DELETE", name, "profile/token/:token_hash", GLEWLWYD_CALLBACK_PRIORITY_APPLICATION, &callback_oauth2_disable_refresh_token, (void*)*cls) || 
                   config->glewlwyd_callback_add_plugin_endpoint(config, "*", name, "profile/*", GLEWLWYD_CALLBACK_PRIORITY_CLOSE, &callback_oauth2_clean, NULL)) {
